...
public class SCAN {
	...
	public SCAN(int init_disk_cyl, int last_disk_cyl, int init_disk_head, int[] queue, int previous_head) {
...
//						System.out.println("index pos" + index_position);
				...
				} else { // direction went right
					if (index_position == (queue_with_head.length - 1)) { //getting to last index and performing operation between last disk head and last num in arr
						right = Math.abs(queue_with_head[index_position] - last_disk_cyl);
						last_unused_right_index = (queue_with_head.length - 1);
					} else if (index_position == queue_with_head.length) { //subtracts last disk c with last unused left index
						left = Math.abs(last_disk_cyl - queue_with_head[last_unused_left_index]);
						direction = -1;
